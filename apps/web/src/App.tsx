...
function SettingsView({ settings, onSaved }: { settings: AppSettings | null; onSaved: () => void }) {
  ...
  async function save(event: FormEvent) {
    ...
    try { await api('/api/settings', { method: 'PATCH', body: JSON.stringify({ geminiKey: key || undefined, billingAcknowledged: ack }) }); setKey(''); setMessage('Settings saved.'); onSaved(); }
  ...
  return <section className="settings-view"><div className="settings-heading"><div><span className="eyebrow">Keeptrail / Settings</span><h1>Keep your setup clear.</h1><p>Local storage and transcription stay on this Mac. Selected text and captures are sent to Google through the managed local gateway when you configure a key.</p></div><Gauge size={29} /></div><form onSubmit={save} className="settings-grid"><section className="settings-card"><div className="settings-card-heading"><div className="settings-icon"><Sparkles size={17} /></div><div><h2>Provider health</h2><p>One Gemini Developer API key, stored locally and never shown again.</p></div></div><label htmlFor="gemini-key">Gemini API key</label><div className="key-input"><input id="gemini-key" type="password" value={key} onChange={(event) => setKey(event.target.value)} placeholder={settings?.hasGeminiKey ? 'Configured · enter a new key to replace' : 'Paste a key from Google AI Studio'} autoComplete="new-password" /><span>{settings?.hasGeminiKey ? 'Configured' : 'Not configured'}</span></div><label className="check-row"><input type="checkbox" checked={ack} onChange={(event) => setAck(event.target.checked)} /><span>This key belongs to a Google AI Studio project without billing enabled.</span></label><p className="settings-note">Keeptrail cannot verify billing state from a pasted key. Free-tier handling follows Google’s terms.</p><button className="button button-primary" disabled={saving || (!key && !settings?.hasGeminiKey)}>{saving ? <LoaderCircle className="spin" size={15} /> : <Check size={15} />} Save and test</button></section><section className="settings-card"><div className="settings-card-heading"><div className="settings-icon"><FolderOpen size={17} /></div><div><h2>Storage and processing</h2><p>Durable local files, resumable jobs, and a conservative cloud cap.</p></div></div><div className="setting-row"><span>Data directory</span><code>{settings?.dataDirectory ?? 'Loading…'}</code></div><div className="setting-row"><span>Cloud calls today</span><strong>{settings?.cloudCallsToday ?? 0} / {settings?.dailyCloudCap ?? 30}</strong></div><div className="setting-row"><span>Selected model</span><strong>{settings?.model ?? 'gemini-2.5-flash-lite'}</strong></div><p className="settings-note">The daily cap can be lowered below 30. Changing the model or increasing the cap is outside the MVP.</p><button type="button" className="button button-quiet" onClick={onSaved}><Archive size={15} />Review storage</button></section><section className="settings-card"><div className="settings-card-heading"><div className="settings-icon"><Globe2 size={17} /></div><div><h2>Browser session</h2><p>Optional, off by default. Use only for sources that need an explicit session.</p></div></div><label className="check-row"><input type="checkbox" checked={settings?.browserSessionEnabled ?? false} onChange={(event) => { void setBrowserSetting({ browserSessionEnabled: event.target.checked }); }} /><span>Allow browser session access when a source asks for it</span></label><div className="browser-choice"><button type="button" className={settings?.browserName === 'chrome' ? 'selected' : ''} onClick={() => { void setBrowserSetting({ browserName: 'chrome' }); }}>Chrome</button><button type="button" className={settings?.browserName === 'firefox' ? 'selected' : ''} onClick={() => { void setBrowserSetting({ browserName: 'firefox' }); }}>Firefox</button></div></section></form>{message && <p className="settings-message" role="status"><Check size={15} />{message}</p>}</section>;
}
