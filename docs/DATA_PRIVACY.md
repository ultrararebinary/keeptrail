...
- The Gemini key, stored as `config/gemini.key` with filesystem mode 0600 when configured.
...
Selected text and captures may be sent through the configured local gateway to Google Gemini for analysis. The app explains this in Settings and on the filter rail. The current worker does not make that cloud call yet; cloud-dependent jobs remain visibly gated.
